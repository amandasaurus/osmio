...
pub struct HeaderBlock<'a> {
    pub bbox: Option<OSMPBF::HeaderBBox>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.bbox = Some(r.read_message::<OSMPBF::HeaderBBox>(bytes)?),
...
pub struct PrimitiveBlock<'a> {
    pub stringtable: OSMPBF::StringTable<'a>,
    pub primitivegroup: Vec<OSMPBF::PrimitiveGroup>,
    pub granularity: i32,
    pub lat_offset: i64,
    pub lon_offset: i64,
...
impl<'a> MessageRead<'a> for PrimitiveBlock<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.stringtable = r.read_message::<OSMPBF::StringTable>(bytes)?,
                Ok(18) => msg.primitivegroup.push(r.read_message::<OSMPBF::PrimitiveGroup>(bytes)?),
...
pub struct PrimitiveGroup {
    pub nodes: Vec<OSMPBF::Node>,
    pub dense: Option<OSMPBF::DenseNodes>,
    pub ways: Vec<OSMPBF::Way>,
    pub relations: Vec<OSMPBF::Relation>,
    pub changesets: Vec<OSMPBF::ChangeSet>,
}
...
impl<'a> MessageRead<'a> for PrimitiveGroup {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.nodes.push(r.read_message::<OSMPBF::Node>(bytes)?),
                Ok(18) => msg.dense = Some(r.read_message::<OSMPBF::DenseNodes>(bytes)?),
                Ok(26) => msg.ways.push(r.read_message::<OSMPBF::Way>(bytes)?),
                Ok(34) => msg.relations.push(r.read_message::<OSMPBF::Relation>(bytes)?),
                Ok(42) => msg.changesets.push(r.read_message::<OSMPBF::ChangeSet>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(34) => msg.info = Some(r.read_message::<OSMPBF::Info>(bytes)?),
...
pub struct DenseNodes {
    pub id: Vec<i64>,
    pub denseinfo: Option<OSMPBF::DenseInfo>,
    pub lat: Vec<i64>,
    pub lon: Vec<i64>,
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(42) => msg.denseinfo = Some(r.read_message::<OSMPBF::DenseInfo>(bytes)?),
...
pub struct Relation {
    ...
    pub memids: Vec<i64>,
    pub types: Vec<OSMPBF::mod_Relation::MemberType>,
}
